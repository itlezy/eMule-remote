...
import { loadConfig, UI_SESSION_COOKIE } from './config.js';
...
function hasValidBearerToken(authHeader: string | undefined): boolean {
  if (!authHeader) {
    return false;
  }

  const prefix = 'Bearer ';
  return authHeader.startsWith(prefix) && authHeader.slice(prefix.length) === config.bearerToken;
}
...
const hashesSchema = z.object({
  hashes: z.array(z.string().trim().min(1)).min(1),
});
...
const addLinksSchema = z.object({
  links: z.array(z.string().trim().min(1)).min(1),
});
...
app.get('/api/v1/log', async (request) => {
  ...
  return callPipe('log/get', { limit: Number.isFinite(limit) ? limit : 200 });
});
