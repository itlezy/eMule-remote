<script lang="ts">
  ...
  const tabClass = (section: Section) => activeSection === section ? 'btn btn-primary' : 'btn';
...
<main class="app-shell">
  <div class="mx-auto max-w-[1800px] px-4 py-6 md:px-6">
      ...
      <div class="panel-body">
        <div class="flex flex-wrap gap-2">
          {#each sections as section}
            <button class={tabClass(section)} type="button" on:click={() => activeSection = section}>{section}</button>
          {/each}
        </div>
      </div>
    </section>
    ...
    {#if activeSection === 'transfers'}
      <div class="grid gap-6 xl:grid-cols-[minmax(0,1.7fr)_minmax(360px,1fr)]">
        <section class="panel">
